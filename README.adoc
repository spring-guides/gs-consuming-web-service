...
browser. Save the file locally as you will need it later.
...
`http://localhost:8080/ws/countries.wsdl`. If you haven't saved it yet, do so now.

Create a `src/main/wsdl` directory and  copy `countries.wsdl` to that location.

Let's configure a build plugin in order to generate classes for the WSDL found in `src/main/wsdl`,
